...
describe('/api/yamato-settings', () => {
  ...
  describe('GET /api/yamato-settings', () => {
    it('should return yamato settings for authenticated user', async () => {
      ...
      validateSession.mockResolvedValue({
        user: mockUser,
        session: { csrf_token: 'csrf-token', session_token: 'session-token' },
      });
  ...
  describe('PUT /api/yamato-settings', () => {
    ...
    it('should sanitize input data', async () => {
      ...
      expect(savedSettings.some((call) => call[1] && call[1].includes('<script>'))).toBe(false); // Should not contain script tags
    });
    ...
    it('should handle database transaction errors', async () => {
      ...
      mockClient.query = jest.fn().mockRejectedValue(new Error('Transaction failed'));
    ...
    it('should validate setting key format', async () => {
      ...
      const insertCalls = mockClient.query.mock.calls.filter(
        (call) => call[0] && call[0].includes('INSERT INTO user_settings')
      );

      // Should save the valid sender_name but not invalid keys
      expect(insertCalls.some((call) => call[1] && call[1].includes('yamato_sender_name'))).toBe(
        true
      );
    });
