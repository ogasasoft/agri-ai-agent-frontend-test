...
describe('/api/admin/dashboard/stats', () => {
  ...
  describe('GET /api/admin/dashboard/stats', () => {
    it('should return system statistics for admin user', async () => {
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        'SELECT COUNT(*) FROM api_integrations WHERE is_active = true'
      );
      expect(mockClient.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT COUNT(*) FROM orders')
      );
    });
    ...
    it('should handle zero counts gracefully', async () => {
      ...
      mockClient.query = jest.fn().mockResolvedValue({ rows: [{ count: '0' }] });
    ...
    it('should format lastBackup as ISO string', async () => {
      ...
      mockClient.query = jest.fn().mockResolvedValue({ rows: [{ count: '1' }] });
    ...
    it('should work with regular admin (not super admin)', async () => {
      ...
      mockClient.query = jest.fn().mockResolvedValue({ rows: [{ count: '5' }] });
    ...
    it('should validate admin session token from cookies', async () => {
      ...
      mockClient.query = jest.fn().mockResolvedValue({ rows: [{ count: '10' }] });
