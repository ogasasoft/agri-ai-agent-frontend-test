...
jest.mock('@/lib/db', () => ({
  getDbClient: jest.fn().mockImplementation(async () => MockDbClient.getInstance()),
}));
...
describe('/api/categories', () => {
  ...
  describe('GET /api/categories', () => {
    ...
    it('should only return categories for current user', async () => {
      ...
      mockClient.setMockData(
        'categories',
        allCategories.filter((c) => c.user_id === 1)
      );
  ...
  describe('POST /api/categories', () => {
    ...
    it('should prevent duplicate category names for same user', async () => {
      ...
      mockClient.setMockData('categories', [createMockCategory({ name: '野菜', user_id: 1 })]);
  ...
  describe('DELETE /api/categories', () => {
    ...
    it('should prevent deletion of category with associated orders', async () => {
      ...
      expect(data.message).toBe(
        'このカテゴリには注文データが関連付けられているため削除できません。'
      );
    });
