    ...
    it('開発環境でdebug_infoが存在することを検証する', () => {
      ...
      const response: StructuredErrorResponse = {
        ...
        debug_info: {
          ...
          processing_steps: [{ step: 'step1', status: 'completed' }],
        },
      };
  ...
  describe('validateUserActions', () => {
    ...
    it('無効なアクションタイプを検証に失敗する', () => {
      const response: StructuredErrorResponse = {
        ...
        message: 'Test',
        error_code: 'TEST',
        user_actions: [{ label: 'Test', action: 'navigate', params: { url: '/home' } }],
      };
  ...
  describe('validateCompleteErrorResponse', () => {
    it('完全なエラーレスポンスを検証する', () => {
      const response: StructuredErrorResponse = {
        ...
        debug_info: {
          timestamp: '2024-01-01T00:00:00Z',
          processing_steps: [{ step: 'auth', status: 'completed' }],
        },
        suggestions: ['check settings'],
        user_actions: [{ label: 'Retry', action: 'retry' }],
      };
