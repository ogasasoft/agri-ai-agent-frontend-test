...
- **ヤマト運輸API** - 配送ラベル作成（開発モック実装済み）
- **カラーミーショップAPI** - 注文同期（計画中）
- **食べチョクAPI** - 注文同期（計画中）
...
- LATE_NOTE.md の内容を README.md に統合
...
- **現状**: 開発モック実装中
- **実装場所**: `src/app/api/yamato/route.ts`
- **優先度**: 中（重要な機能だが、クレデンシャル取得前にスキップ可能）
- **本格実装時**: クレデンシャルが揃ったら実際のAPIに置き換える

### 週次成長計算

- **現状**: `weeklyGrowth: 0` で固定（`src/app/api/admin/dashboard/stats/route.ts`）
- **優先度**: 低（非必須機能）
- **実装計画中**

### 配送ステータス追跡

- **現状**: モック実装中（`src/app/api/shipping/route.ts`）
- **優先度**: 中
- **本格実装時**: APIキー設定後に実際のヤマトAPIに置き換える

### 管理者エンドポイント

- **現状**: `/api/admin/*` エンドポイントは実装済み
- **優先度**: 低（セキュリティ上は良い実装）
- **使用方針**: スーパー管理者は `/api/admin/*` を使用すべき

### 外部連携（今後実装予定）

- **カラーミーショップAPI**: 商品・注文自動同期（計画中）
- **食べチョクAPI**: 注文データ取得（計画中）
- **ヤマト運輸API**: 配送ラベル自動作成（開発モック実装済み）

### 開発ロードマップ

- [x] ドキュメント統合（LATE_NOTE.md → README.md）
- [ ] ヤマト運輸API連携の本格実装
- [ ] 週次成長計算ロジックの実装
- [ ] 配送ステータス追跡の本格実装
- [ ] カラーミーショップAPI連携
