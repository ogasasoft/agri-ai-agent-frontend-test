...
### 実装状況 (Integration Status)

- **ヤマト運輸API連携**: 開発モック実装中 (`src/app/api/yamato/route.ts`)。クレデンシャルが揃い次第、本格実装予定。
- **配送ステータス追跡**: モック実装中 (`src/app/api/shipping/route.ts`)。APIキー設定後、実際のヤマトAPIに置き換え予定。
- **カラーミーショップAPI**: 計画中
- **食べチョクAPI**: 計画中
...
- **専用エンドポイント**: スーパー管理者は `/api/admin/*` エンドポイントを使用して管理機能にアクセスします。
