...
export function getClientInfo(request: NextRequest): { ipAddress: string; userAgent: string } {
  // Next.js 15+ removed request.ip, so we need to get it from headers
...
export async function authenticateUserEnhanced(
  ...
  try {
    ...
    if (!isPasswordValid) {
      ...
      const newLockoutLevel = Math.min(
        Math.floor(newFailures / MAX_LOGIN_ATTEMPTS),
        PROGRESSIVE_LOCKOUT_LEVELS.length - 1
      );
    ...
    await logAuditEvent(
      ...
      'user',
      user.id,
      { session_id: session.id, remember_me: rememberMe },
      ipAddress,
      userAgent
    );
...
export async function autoLoginWithRememberToken(
  ...
  try {
    ...
    if (!isValidToken) {
      ...
      return {
        success: false,
        message: 'セキュリティ上の理由によりログアウトしました。再度ログインしてください。',
      };
    }
...
export async function validateRememberToken(
  selector: string,
  validator: string
): Promise<{ user: User; newSession: Session } | null> {
  const client = await getDbClient();
  ...
  try {
    ...
    await logAuditEvent(tokenData.user_id, 'AUTO_LOGIN_SUCCESS', 'user', tokenData.user_id, {
      remember_token: true,
    });
...
export async function validateSessionEnhanced(
  sessionToken: string,
