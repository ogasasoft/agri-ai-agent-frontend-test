...
export const useErrorHandler = (options: UseErrorHandlerOptions = {}) => {
  ...
  const handleFormError = useCallback(
    (
      ...
      if (type === 'validation') {
        errorDetails = FormErrorBuilder.validationError(formName, error, formData || {}, context);
      } else {
        errorDetails = FormErrorBuilder.submissionError(formName, error, formData || {}, context);
      }
  ...
  const handleApiError = useCallback(
    (endpoint: string, error: any, additionalContext?: Record<string, any>) => {
      const context = {
        componentName,
        currentPage: window.location.pathname,
        userAction: 'api_call',
...
export const useFormErrorHandler = (formName: string, options: UseErrorHandlerOptions = {}) => {
  ...
  const handleValidationError = useCallback(
    (validationErrors: Record<string, string[]>, formData: Record<string, any>) => {
  ...
  const handleSubmissionError = useCallback(
    (error: any, formData: Record<string, any>) => {
...
export const useApiErrorHandler = (options: UseErrorHandlerOptions = {}) => {
  ...
  const handleFetchError = useCallback(
    async (fetchFunction: () => Promise<any>, endpoint: string) => {
      try {
