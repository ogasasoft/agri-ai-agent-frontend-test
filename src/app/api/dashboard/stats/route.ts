  ...
  try {
    ...
    try {
      ...
      const previousFromDate = new Date(
        new Date(fromDate).getTime() - dateDiff * 24 * 60 * 60 * 1000
      )
        .toISOString()
        .split('T')[0];
      const previousToDate = new Date(new Date(toDate).getTime() - dateDiff * 24 * 60 * 60 * 1000)
        .toISOString()
        .split('T')[0];
    ...
    const dbError = DatabaseErrorBuilder.queryError('SELECT dashboard stats', error, {
      table: 'orders',
      operation: 'SELECT',
      userId: userId,
    });
