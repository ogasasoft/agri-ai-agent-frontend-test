...
const generateYamatoB2CSV = (orders: any[]): string => {
  ...
  const rows = orders.map((order, index) => {
    const trackingNumber =
      order.tracking_number || `AG${Date.now()}${String(index + 1).padStart(3, '0')}`;
    ...
    try {
      ...
      for (let i = 0; i < selectedOrders.length; i++) {
        ...
        try {
          ...
          `,
            [shippedAt, trackingNumber, order.id, userId]
          );
...
export async function GET(request: NextRequest) {
  try {
    ...
    // NOTE: Mock delivery status - this will be replaced with actual Yamato API delivery tracking when API keys are configured
