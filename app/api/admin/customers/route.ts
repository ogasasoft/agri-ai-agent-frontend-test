...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/admin/customers`,
      {
        headers: {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!customer_id) {
      return NextResponse.json({ success: false, message: '顧客IDは必須です。' }, { status: 400 });
    }

    const result = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/admin/customers?id=${customer_id}`,
      {
        method: 'DELETE',
        headers: {
          'x-session-token': token || '',
